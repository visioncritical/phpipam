...
var prettyLinks=$('#prettyLinks').html();if(prettyLinks=="Yes"){window.location=base+"tools/search/"+ip;}
else{window.location=base+"?page=tools&section=search&ip="+ip;}
return false;});$('form#userMenuSearch').submit(function(){showSpinner();var ip=$('.searchInput').val();var ua=window.navigator.userAgent;var msie=ua.indexOf("MSIE ");if(msie>0||!!navigator.userAgent.match(/Trident.*rv\:11\./)){var base=$('.iebase').html();}
else{var base="";}
var prettyLinks=$('#prettyLinks').html();if(prettyLinks=="Yes"){window.location=base+"tools/search/"+ip;}
else{window.location=base+"?page=tools&section=search&ip="+ip;}
return false;});$('form#search').submit(function(){showSpinner();var ip=$('form#search .search').val();var prettyLinks=$('#prettyLinks').html();if(prettyLinks=="Yes"){window.location="tools/search/"+ip;}
else{window.location="?page=tools&section=search&ip="+ip;}
return false;});$('a#exportSearch').click(function(){var searchTerm=$('form#search .search').val();$("div.dl").remove();$('div.exportDIVSearch').append("<div style='display:none' class='dl'><iframe src='site/tools/searchResultsExport.php?searchTerm="+searchTerm+"'></iframe></div>");return false;});$('#hosts').submit(function(){showSpinner();var hostname=$('input.hostsFilter').val();var prettyLinks=$('#prettyLinks').html();if(prettyLinks=="Yes"){window.location=base+"tools/hosts/"+hostname;}
...
else{hideSpinner();hideSpinner();}}).fail(function(jqxhr,textStatus,errorThrown){showError(jqxhr.statusText+"<br>Status: "+textStatus+"<br>Error: "+errorThrown);});return false;});$('.editVLAN').click(function(){showSpinner();var vlanId=$(this).attr('data-vlanid');var action=$(this).attr('data-action');if($(this).attr("data-number")){var vlanNum=$(this).attr('data-number');}
else{var vlanNum="";}
